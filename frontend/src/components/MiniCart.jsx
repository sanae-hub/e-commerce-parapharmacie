    ...
    return (
      <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-gray-200 z-50 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-900">{t('cart.mini_title')}</h3>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">{t('cart.mini_empty')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="absolute right-0 mt-2 w-96 bg-white rounded-lg shadow-xl border border-gray-200 z-50">
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <h3 className="text-lg font-bold text-gray-900">{t('cart.mini_title')} ({cartItems.length})</h3>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-600">
          <X size={20} />
        </button>
      </div>
            ...
            <div key={`${item.id}-${item.variantId || ''}`} className="flex gap-3 p-3 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-xs text-gray-600">{t('common.quantity_short')}: {item.quantity}</span>
                  <span className="text-sm font-bold text-sky-700 ltr">
                    {(item.price * item.quantity).toFixed(2)} DH</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="p-4 border-t border-gray-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-xl font-bold text-sky-700 ltr">{getTotalPrice().toFixed(2)} DH</span>
        </div>
        <button
